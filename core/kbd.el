...
(defvar kbd-mode-map
  (let ((map (make-sparse-keymap)))
    (let ((prefix-map (make-sparse-keymap)))
      ...
      (define-key prefix-map (kbd "r i") 'phpactor-implement-contracts)
      (define-key prefix-map (kbd "r g t") 'phpactor-goto-definition)
      (define-key prefix-map (kbd "r m c") 'phpactor-move-class)
      (define-key prefix-map (kbd "r e c") 'phpactor-extract-constant)
      (define-key prefix-map (kbd "r e e") 'phpactor-extract-expression)
      (define-key prefix-map (kbd "r e m") 'phpactor-extract-method)
      (define-key prefix-map (kbd "r c c") 'phpactor-complete-constructor)
      (define-key prefix-map (kbd "r c r") 'phpactor-move-class)
      (define-key prefix-map (kbd "r r") 'phpactor-replace-references)
      (define-key prefix-map (kbd "r v") 'phpactor-rename-variable-local)
